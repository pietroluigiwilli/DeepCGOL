...
def torch_evolve(frame, device, generations=1):
    # The results of this function are not the same as the evolve function above.
	# This function is used to evolve a frame using convolution. it doesn't pad in a circular manner.
	# In other words, it doesn't wrap around the edges of the frame.
    frame = frame.float()
    kernel = torch.tensor([[1., 1., 1.], [1., 0., 1.], [1., 1., 1.]]).float().to(device)
    neighbours = torch.conv2d(frame.unsqueeze(0).unsqueeze(0), kernel.unsqueeze(0).unsqueeze(0), padding=1).squeeze().int()
    return ((neighbours == 3) | (frame.int() & (neighbours == 2))).int()
